...
from vnpy.trader.mcdata import mcdata_client
...
class CtaEngine(BaseEngine):
    ...
    def init_mcdata(self):
        """
        Init mcdata client.
        """
        result = mcdata_client.init()
        if result:
            self.write_log("mcdata数据接口初始化成功")

    def query_bar_from_rq(
        self, symbol: str, exchange: Exchange, interval: Interval, start: datetime, end: datetime
    ):
        """
        Query bar data from mcdata.
        """
        req = HistoryRequest(
            symbol=symbol,
            exchange=exchange,
        ...
        data = mcdata_client.query_history(req)
        return data
    ...
    def load_bar(
        ...
        # Pass gateway and mcdata if use_database set to True
        if not use_database:
            ...
            # Try to query bars from mcdata, if not found, load from database.
            else:
    ...
    def add_strategy(
        ...
        _, exchange_str = extract_vt_symbol(vt_symbol)
        if exchange_str not in Exchange:
